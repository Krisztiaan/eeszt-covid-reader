/**
 * From expo, slight modifications to comply with lint rules.
 *
 * @see https://github.com/expo/expo/blob/master/home/components/Camera.android.tsx
 */
...
export function Camera(props: OriginalCamera['props']): JSX.Element {
  ...
  return (
    ...
    >
      <OriginalCamera
        ...
        style={
          suggestedDimensions && width && height && dimensions
            ? {
                position: 'absolute',
                width,
                height,
...
function useAutoSize(
  ...
  useEffect(() => {
    const calcSuggestedAspectRatio = findClosestAspectRatio(supportedAspectRatios, dimensions);
    const calcSuggestedDimensions = calculateSuggestedDimensions(
      dimensions,
      calcSuggestedAspectRatio,
    );

    if (!calcSuggestedAspectRatio || !calcSuggestedDimensions) {
      setSuggestedAspectRatio(null);
      setSuggestedDimensions(null);
    } else {
      setSuggestedAspectRatio(calcSuggestedAspectRatio);
      setSuggestedDimensions(calcSuggestedDimensions);
    }
...
function useSupportedAspectRatios(): [string[] | null, CameraRefCallback] {
  ...
  const ref = useCallback(
    (node: CameraRef | null) => {
      if (node !== null) {
        node.getSupportedRatiosAsync().then(setAspectRatios).catch(console.error);
      }
    },
    [setAspectRatios],
  );
...
const useComponentDimensions = (): [Dimensions | null, (e: LayoutChangeEvent) => void] => {
